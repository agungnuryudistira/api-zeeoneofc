...
var creatorList = ['execllent','agungnur.y'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di line : bit.ly/agung-tampan , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------AgungGanteng'+'AGUNG--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'Noel-Operation') return res.json(loghandler.invalidKey)
...
router.get('/tiktokdl', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Noel-Operation') return res.json(loghandler.invalidKey)
     if (!url) return res.json(loghandler.noturl)

     fetch(encodeURI(`https://godownloader.com/api/tiktok-no-watermark-free?url=${url}&key=godownloader.com`))
...
router.get('/ig/stalk', async (req, res, next) => {
    ...
    if (!username) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter username"})
       ...
       fetch(encodeURI(`https://instagram-bulk-profile-scrapper.p.rapidapi.com/clients/api/ig/bulk_profile?response_type=full&ig=${username}&corsEnabled=true`,{`method`:`GET`,`headers`:{`"x-rapidapi-key`:`4d02e53424msh16fdc401cf915abp1fa242jsnd326999f80f6`,`"x-rapidapi-host`:`instagram-bulk-profile-scrapper.p.rapidapi.com`}}))
        .then(response => response.json())
        .then(data => {
        var isinya = data;
             res.json({
